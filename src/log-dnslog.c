                            ...
                            PacketQueue *postpq, int ipproto)
{
    ...
    uint64_t total_txs = AlpGetTxCnt(p->flow->proto, proto, dns_state);
    uint64_t tx_id = AlpGetTransactionLogId(p->flow->alparser);
    ...
    {
        tx = AlpGetTx(p->flow->proto, proto, dns_state, tx_id);
        if (tx == NULL)
            continue;
...
OutputCtx *LogDnsLogInitCtx(ConfNode *conf)
{
    ...
    AlpRegisterLogger(IPPROTO_UDP, ALPROTO_DNS);
    AlpRegisterLogger(IPPROTO_TCP, ALPROTO_DNS);
