                                ...
                                void *local_data)
{
    ...
    if (AlpParserStateIssetFlag(pstate, APP_LAYER_PARSER_EOF) &&
                                  ...
                                  void *tx, uint8_t *chunks_buffer, uint32_t chunks_buffer_len)
{
    ...
    tx_progress = AlpGetStateProgress(IPPROTO_TCP, ALPROTO_HTTP, tx, 0);
...
void HTPFreeConfig(void)
{
    ...
    if (!AlpdConfProtoDetectionEnabled("http") || !AlpConfParserEnabled("http"))
        SCReturn;
...
static int HTPCallbackRequest(htp_tx_t *tx) {
     ...
     * at the same time. */
    AlpTriggerRawStreamReassembly(hstate->f);
    SCReturnInt(HTP_OK);
}
...
{
    /* toserver */
    if (AlpdPMRegisterPatternCS(IPPROTO_TCP, ALPROTO_HTTP,
...
void RegisterHTPParsers(void)
{
    ...
    if (AlpdConfProtoDetectionEnabled(proto_name)) {
        if (AlpdRegisterProtocol(ALPROTO_HTTP, proto_name) < 0)
            return;
        if (HTPRegisterPatternsForProtocolDetection() < 0)
            return;
        AlpRegisterParserAcceptableDataDirection(IPPROTO_TCP, ALPROTO_HTTP, STREAM_TOSERVER);
    } else {
    ...
    if (AlpConfParserEnabled(proto_name)) {
        AlpRegisterStateFuncs(IPPROTO_TCP, ALPROTO_HTTP, HTPStateAlloc, HTPStateFree);
        AlpRegisterTxFreeFunc(IPPROTO_TCP, ALPROTO_HTTP, HTPStateTransactionFree);
        AlpRegisterGetFilesFunc(IPPROTO_TCP, ALPROTO_HTTP, HTPStateGetFiles);
        AlpRegisterGetStateProgressFunc(IPPROTO_TCP, ALPROTO_HTTP, HTPStateGetAlstateProgress);
        AlpRegisterGetTxCnt(IPPROTO_TCP, ALPROTO_HTTP, HTPStateGetTxCnt);
        AlpRegisterGetTx(IPPROTO_TCP, ALPROTO_HTTP, HTPStateGetTx);
        AlpRegisterGetStateProgressCompletionStatus(IPPROTO_TCP, ALPROTO_HTTP,
                                                      HTPStateGetAlstateProgressCompletionStatus);

        AlpRegisterGetEventInfo(IPPROTO_TCP, ALPROTO_HTTP, HTPStateGetEventInfo);

        AlpRegisterTruncateFunc(IPPROTO_TCP, ALPROTO_HTTP, HTPStateTruncate);

        AlpRegisterParser(IPPROTO_TCP, ALPROTO_HTTP, STREAM_TOSERVER,
                          HTPHandleRequestData);
        AlpRegisterParser(IPPROTO_TCP, ALPROTO_HTTP, STREAM_TOCLIENT,
...
int HTPParserTest01(void) {
    ...
    for (u = 0; u < httplen1; u++) {
        ...
        SCMutexLock(&f->m);
        r = AlpParseL7Data(app_tctx, f, ALPROTO_HTTP, flags, &httpbuf1[u], 1);
        if (r != 0) {
...
int HTPParserTest02(void) {
    ...
    SCMutexLock(&f->m);
    int r = AlpParseL7Data(app_tctx, f, ALPROTO_HTTP, STREAM_TOSERVER|STREAM_START|
...
int HTPParserTest04(void) {
    ...
    SCMutexLock(&f->m);
    r = AlpParseL7Data(app_tctx, f, ALPROTO_HTTP, STREAM_TOSERVER|STREAM_START|
                          STREAM_EOF, httpbuf1, httplen1);
    if (r != 0) {
        SCMutexUnlock(&f->m);
        goto end;
...
int HTPParserTest05(void) {
    ...
    r = AlpParseL7Data(app_tctx, f, ALPROTO_HTTP, STREAM_TOCLIENT|STREAM_START, httpbuf4,
                      httplen4);
    if (r != 0) {
    ...
    r = AlpParseL7Data(app_tctx, f, ALPROTO_HTTP, STREAM_TOCLIENT, httpbuf5, httplen5);
    if (r != 0) {
    ...
    r = AlpParseL7Data(app_tctx, f, ALPROTO_HTTP, STREAM_TOSERVER|STREAM_EOF, httpbuf3,
                      httplen3);
    if (r != 0) {
    ...
    r = AlpParseL7Data(app_tctx, f, ALPROTO_HTTP, STREAM_TOCLIENT|STREAM_EOF, httpbuf6,
                      httplen6);
    if (r != 0) {
...
int HTPParserTest06(void) {
    ...
    r = AlpParseL7Data(app_tctx, f, ALPROTO_HTTP, STREAM_TOCLIENT|STREAM_START, httpbuf2,
                      httplen2);
    if (r != 0) {
...
";
    ...
    SCMutexLock(&f->m);
    r = AlpParseL7Data(app_tctx, f, ALPROTO_HTTP, flags, httpbuf1, httplen1);
    if (r != 0) {
...
static int HTPBodyReassemblyTest01(void)
{
    ...
    void *parser = AlpAllocAlpParserState();
...
";
    ...
    SCMutexLock(&f->m);
    r = AlpParseL7Data(app_tctx, f, ALPROTO_HTTP, STREAM_TOSERVER, httpbuf1, httplen1);
    if (r != 0) {
        printf("toserver chunk 1 returned %" PRId32 ", expected 0: ", r);
        result = 0;
        SCMutexUnlock(&f->m);
    ...
    SCMutexLock(&f->m);
    AppLayerDecoderEvents *decoder_events = AlpGetDecoderEvents(f->alparser);
...
";
    ...
    for (u = 0; u < len; u++) {
        ...
        SCMutexLock(&f->m);
        r = AlpParseL7Data(app_tctx, f, ALPROTO_HTTP, flags, (uint8_t *)&httpbuf[u], 1);
