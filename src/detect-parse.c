                                ...
                                uint8_t dir)
{
    ...
    if (sig->alproto != ALPROTO_UNKNOWN) {
        ...
        if (override_needed)
            AlpdSupportedIpprotos(sig->alproto, sig->proto.proto);
    }
    ...
    if (sig->sm_lists[DETECT_SM_LIST_APP_EVENT] != NULL) {
        if (AlpProtocolIsTxEventAware(IPPROTO_TCP, sig->alproto)) {
        ...
        if (AlpProtocolIsTxEventAware(IPPROTO_UDP, sig->alproto)) {
            if (sig->flags & STREAM_TOSERVER) {
