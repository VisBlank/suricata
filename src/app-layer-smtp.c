...
void RegisterSMTPParsers(void)
{
    ...
    if (AlpdConfProtoDetectionEnabled(proto_name)) {
        if (AlpdRegisterProtocol(ALPROTO_SMTP, proto_name) < 0)
            return;
    ...
    if (AlpConfParserEnabled(proto_name)) {
        AlpRegisterStateFuncs(IPPROTO_TCP, ALPROTO_SMTP, SMTPStateAlloc, SMTPStateFree);

        AlpRegisterParser(IPPROTO_TCP, ALPROTO_SMTP, STREAM_TOSERVER,
                              SMTPParseClientRecord);
        AlpRegisterParser(IPPROTO_TCP, ALPROTO_SMTP, STREAM_TOCLIENT,
                              SMTPParseServerRecord);

        AlpRegisterGetEventInfo(IPPROTO_TCP, ALPROTO_SMTP, SMTPStateGetEventInfo);

        AlpRegisterLocalStorageFunc(IPPROTO_TCP, ALPROTO_SMTP, SMTPLocalStorageAlloc,
...
int SMTPParserTest01(void)
{
    ...
    SCMutexLock(&f.m);
    r = AlpParseL7Data(alp_tctx, &f, ALPROTO_SMTP, STREAM_TOCLIENT,
    ...
    SCMutexLock(&f.m);
    r = AlpParseL7Data(alp_tctx, &f, ALPROTO_SMTP, STREAM_TOSERVER,
...
int SMTPParserTest12(void)
{
    ...
    SCMutexLock(&f.m);
    r = AlpParseL7Data(alp_tctx, &f, ALPROTO_SMTP, STREAM_TOSERVER | STREAM_START,
                      request1, request1_len);
    if (r != 0) {
    ...
    SCMutexLock(&f.m);
    r = AlpParseL7Data(alp_tctx, &f, ALPROTO_SMTP, STREAM_TOCLIENT | STREAM_TOCLIENT,
                      reply1, reply1_len);
    if (r == 0) {
        printf("AppLayerParse for smtp failed.  Returned %" PRId32, r);
        SCMutexUnlock(&f.m);
