                                               ...
                                               uint32_t *stream_start_offset)
{
    ...
    *stream_start_offset = 0;
    ...
    if ((htp_state->cfg->request_body_limit == 0 ||
        ...
        !(AlpGetStateProgress(IPPROTO_TCP, ALPROTO_HTTP, tx, 0) > HTP_REQUEST_BODY) &&
        !(flags & STREAM_EOF)) {
                                      ...
                                      void *alstate, void *tx, uint64_t tx_id)
{
    ...
    if (AlpGetStateProgress(IPPROTO_TCP, ALPROTO_HTTP, tx, 0) > HTP_REQUEST_BODY)
...
#define TOTAL_REQUESTS 45
    ...
    SCMutexLock(&f.m);
    int r = AlpParseL7Data(app_tctx, &f, ALPROTO_HTTP, STREAM_TOSERVER, http_buf, http_buf_len);
    if (r != 0) {
        printf("toserver chunk 1 returned %" PRId32 ", expected 0: ", r);
        SCMutexUnlock(&f.m);
        goto end;
    ...
    SCMutexLock(&f.m);
    r = AlpParseL7Data(app_tctx, &f, ALPROTO_HTTP, STREAM_TOCLIENT,
