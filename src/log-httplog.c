                            ...
                            PacketQueue *postpq, int ipproto)
{
    ...
    total_txs = AlpGetTxCnt(IPPROTO_TCP, ALPROTO_HTTP, htp_state);
    tx_id = AlpGetTransactionLogId(p->flow->alparser);
    tx_progress_done_value_ts = AlpGetStateProgressCompletionStatus(IPPROTO_TCP, ALPROTO_HTTP, 0);
    tx_progress_done_value_tc = AlpGetStateProgressCompletionStatus(IPPROTO_TCP, ALPROTO_HTTP, 1);
    ...
    {
        tx = AlpGetTx(IPPROTO_TCP, ALPROTO_HTTP, htp_state, tx_id);
        if (tx == NULL) {
            SCLogDebug("tx is NULL not logging !!");
            continue;
        }

        if (!AlpParserStateIssetFlag(p->flow->alparser, APP_LAYER_PARSER_EOF)) {
            tx_progress = AlpGetStateProgress(IPPROTO_TCP, ALPROTO_HTTP, tx, 0);
            if (tx_progress < tx_progress_done_value_ts)
                break;

            tx_progress = AlpGetStateProgress(IPPROTO_TCP, ALPROTO_HTTP, tx, 1);
        ...
        AlpSetTransactionLogId(p->flow->alparser);
    }
...
OutputCtx *LogHttpLogInitCtx(ConfNode *conf)
{
    ...
    AlpRegisterLogger(IPPROTO_TCP, ALPROTO_HTTP);
