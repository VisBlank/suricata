...
void RegisterSMB2Parsers(void) {
    ...
    if (AlpdConfProtoDetectionEnabled(proto_name)) {
        AlpRegisterParser(IPPROTO_TCP, ALPROTO_SMB2, STREAM_TOSERVER, SMB2Parse);
        AlpRegisterParser(IPPROTO_TCP, ALPROTO_SMB2, STREAM_TOCLIENT, SMB2Parse);
        AlpRegisterStateFuncs(IPPROTO_TCP, ALPROTO_SMB2, SMB2StateAlloc, SMB2StateFree);
    } else {
...
int SMB2ParserTest01(void) {
    ...
    SCMutexLock(&f.m);
    int r = AlpParseL7Data(app_tctx, &f, ALPROTO_SMB2, STREAM_TOSERVER|STREAM_EOF, smb2buf, smb2len);
    if (r != 0) {
