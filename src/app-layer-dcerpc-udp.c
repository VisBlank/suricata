...
{
    if (AlpdPMRegisterPatternCS(IPPROTO_TCP, ALPROTO_DCERPC,
...
void RegisterDCERPCUDPParsers(void) {
    ...
    if (AlpdConfProtoDetectionEnabled("dcerpc")) {
        if (AlpdRegisterProtocol(ALPROTO_DCERPC, proto_name) < 0) {
        ...
        AlpRegisterParserAcceptableDataDirection(IPPROTO_UDP, ALPROTO_DCERPC, STREAM_TOSERVER);
    } else {
    ...
    if (AlpConfParserEnabled("dcerpc")) {
        AlpRegisterParser(IPPROTO_UDP, ALPROTO_DCERPC, STREAM_TOSERVER,
                          DCERPCUDPParse);
        AlpRegisterParser(IPPROTO_UDP, ALPROTO_DCERPC, STREAM_TOCLIENT,
                          DCERPCUDPParse);
        AlpRegisterStateFuncs(IPPROTO_UDP, ALPROTO_DCERPC, DCERPCUDPStateAlloc,
                              DCERPCUDPStateFree);
    } else {
...
#ifdef UNITTESTS
    //AppLayerParserRegisterUnittests(ALPROTO_DCERPC, DCERPCUDPParserRegisterTests);
#endif
...
int DCERPCUDPParserTest01(void) {
	...
	SCMutexLock(&f.m);
	int r = AlpParseL7Data(app_tctx, &f, ALPROTO_DCERPC, STREAM_TOSERVER|STREAM_START, dcerpcrequest, requestlen);
	if (r != 0) {
