                                    ...
                                    void *txv, uint64_t idx)
{
    ...
    if (flags & STREAM_TOSERVER) {
        if (AlpGetStateProgress(IPPROTO_TCP, ALPROTO_HTTP, txv, 0) <= HTP_REQUEST_HEADERS)
            SCReturnInt(cnt);
        ...
        if (AlpGetStateProgress(IPPROTO_TCP, ALPROTO_HTTP, txv, 1) <= HTP_RESPONSE_HEADERS)
            SCReturnInt(cnt);
...
static int DetectEngineHttpRawHeaderTest01(void)
{
    ...
    SCMutexLock(&f.m);
    int r = AlpParseL7Data(app_tctx, &f, ALPROTO_HTTP, STREAM_TOSERVER, http_buf, http_len);
    if (r != 0) {
...
static int DetectEngineHttpRawHeaderTest20(void)
{
    ...
    SCMutexLock(&f.m);
    int r = AlpParseL7Data(app_tctx, &f, ALPROTO_HTTP, STREAM_TOSERVER, http1_buf, http1_len);
    if (r != 0) {
        printf("toserver chunk 1 returned %" PRId32 ", expected 0: ", r);
        result = 0;
        SCMutexUnlock(&f.m);
    ...
    SCMutexLock(&f.m);
    r = AlpParseL7Data(app_tctx, &f, ALPROTO_HTTP, STREAM_TOSERVER, http2_buf, http2_len);
    if (r != 0) {
...
static int DetectEngineHttpRawHeaderTest28(void)
{
    ...
    SCMutexLock(&f.m);
    int r = AlpParseL7Data(app_tctx, &f, ALPROTO_HTTP, STREAM_TOSERVER, http_buf1,
                          http_buf1_len);
    if (r != 0) {
        printf("toserver chunk 1 returned %" PRId32 ", expected 0: ", r);
        result = 0;
    ...
    SCMutexLock(&f.m);
    r = AlpParseL7Data(app_tctx, &f, ALPROTO_HTTP, STREAM_TOCLIENT, http_buf2, http_buf2_len);
    if (r != 0) {
        printf("toserver chunk 1 returned %" PRId32 ", expected 0: \n", r);
        result = 0;
        SCMutexUnlock(&f.m);
