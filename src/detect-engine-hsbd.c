    ...
    *stream_start_offset = 0;
    ...
    SCLogDebug("response_body_limit %u response_body.content_len_so_far %"PRIu64
              ...
              flags & STREAM_EOF ? "true" : "false",
               (AlpGetStateProgress(IPPROTO_TCP, ALPROTO_HTTP, tx, 1) > HTP_RESPONSE_BODY) ? "true" : "false");
    ...
    if ((htp_state->cfg->response_body_limit == 0 ||
        ...
        !(AlpGetStateProgress(IPPROTO_TCP, ALPROTO_HTTP, tx, 1) > HTP_RESPONSE_BODY) &&
        !(flags & STREAM_EOF)) {
                                      ...
                                      void *tx, uint64_t tx_id)
{
    ...
    if (AlpGetStateProgress(IPPROTO_TCP, ALPROTO_HTTP, tx, 1) > HTP_RESPONSE_BODY)
...
static int DetectEngineHttpServerBodyTest01(void)
{
    ...
    SCMutexLock(&f.m);
    int r = AlpParseL7Data(app_tctx, &f, ALPROTO_HTTP, STREAM_TOSERVER, http_buf1, http_len1);
    if (r != 0) {
    ...
    SCMutexLock(&f.m);
    r = AlpParseL7Data(app_tctx, &f, ALPROTO_HTTP, STREAM_TOCLIENT, http_buf2, http_len2);
    if (r != 0) {
...
static int DetectEngineHttpServerBodyTest03(void)
{
    ...
    SCMutexLock(&f.m);
    r = AlpParseL7Data(app_tctx, &f, ALPROTO_HTTP, STREAM_TOCLIENT, http_buf3, http_len3);
    if (r != 0) {
        printf("toserver chunk 1 returned %" PRId32 ", expected 0: \n", r);
        result = 0;
        SCMutexUnlock(&f.m);
