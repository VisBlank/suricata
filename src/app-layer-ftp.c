...
{
    if (AlpdPMRegisterPatternCI(IPPROTO_TCP, ALPROTO_FTP,
...
void RegisterFTPParsers(void) {
    ...
    if (AlpdConfProtoDetectionEnabled(proto_name)) {
        if (AlpdRegisterProtocol(ALPROTO_FTP, proto_name) < 0)
            return;
        if (FTPRegisterPatternsForProtocolDetection() < 0 )
            return;

        AlpRegisterParserAcceptableDataDirection(IPPROTO_TCP, ALPROTO_FTP, STREAM_TOSERVER | STREAM_TOCLIENT);
    }

    if (AlpConfParserEnabled(proto_name)) {
        AlpRegisterParser(IPPROTO_TCP, ALPROTO_FTP, STREAM_TOSERVER,
                          FTPParseRequest);
        AlpRegisterParser(IPPROTO_TCP, ALPROTO_FTP, STREAM_TOCLIENT,
                          FTPParseResponse);
        AlpRegisterStateFuncs(IPPROTO_TCP, ALPROTO_FTP, FTPStateAlloc, FTPStateFree);
    } else {
        SCLogInfo("Parsed disabled for %s protocol. Protocol detection"
                  "still on.", proto_name);
    }
#ifdef UNITTESTS
    //AlpParseL7DatarRegisterUnittests(ALPROTO_FTP, FTPParserRegisterTests);
#endif
}
...
int FTPParserTest01(void) {
    ...
    SCMutexLock(&f.m);
    int r = AlpParseL7Data(app_tctx, &f, ALPROTO_FTP, STREAM_TOSERVER|STREAM_EOF, ftpbuf, ftplen);
    if (r != 0) {
...
int FTPParserTest03(void) {
    ...
    SCMutexLock(&f.m);
    int r = AlpParseL7Data(app_tctx, &f, ALPROTO_FTP, STREAM_TOSERVER|STREAM_START, ftpbuf1, ftplen1);
    if (r != 0) {
        SCLogDebug("toserver chunk 1 returned %" PRId32 ", expected 0: ", r);
        result = 0;
        SCMutexUnlock(&f.m);
    ...
    SCMutexLock(&f.m);
    r = AlpParseL7Data(app_tctx, &f, ALPROTO_FTP, STREAM_TOSERVER, ftpbuf2, ftplen2);
    if (r != 0) {
    ...
    SCMutexLock(&f.m);
    r = AlpParseL7Data(app_tctx, &f, ALPROTO_FTP, STREAM_TOSERVER|STREAM_EOF, ftpbuf3, ftplen3);
    if (r != 0) {
...
int FTPParserTest06(void) {
    ...
    SCMutexLock(&f.m);
    int r = AlpParseL7Data(app_tctx, &f, ALPROTO_FTP, STREAM_TOSERVER|STREAM_START|STREAM_EOF, ftpbuf1, ftplen1);
    if (r != 0) {
        SCLogDebug("toserver chunk 1 returned %" PRId32 ", expected 0: ", r);
        result = 0;
        SCMutexUnlock(&f.m);
...
int FTPParserTest07(void) {
    ...
    SCMutexLock(&f.m);
    r = AlpParseL7Data(app_tctx, &f, ALPROTO_FTP, STREAM_TOSERVER|STREAM_EOF, ftpbuf2, ftplen2);
    if (r != 0) {
        SCLogDebug("toserver chunk 2 returned %" PRId32 ", expected 0: ", r);
        result = 0;
        SCMutexUnlock(&f.m);
...
int FTPParserTest10(void) {
    ...
    for (u = 0; u < ftplen1; u++) {
        ...
        SCMutexLock(&f.m);
        r = AlpParseL7Data(app_tctx, &f, ALPROTO_FTP, flags, &ftpbuf1[u], 1);
        if (r != 0) {
