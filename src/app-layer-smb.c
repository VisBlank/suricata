...
{
    if (AlpdPMRegisterPatternCS(IPPROTO_TCP, ALPROTO_SMB,
                                "|ff|SMB", 8, 4, STREAM_TOSERVER) < 0)
        {
            return -1;
        }
    if (AlpdPMRegisterPatternCS(IPPROTO_TCP, ALPROTO_SMB2,
...
void RegisterSMBParsers(void) {
    ...
    if (AlpdConfProtoDetectionEnabled(proto_name)) {
        if (AlpdRegisterProtocol(ALPROTO_SMB, proto_name) < 0)
            return;
        ...
        //                              SMBProbingParser);
        //} else {
        //    AlpdParseProbingParserPorts(proto_name, ALPROTO_SMB,
        //                                SMB_PROBING_PARSER_MIN_DEPTH, 0,
        //                                SMBProbingParser);
        //}

        AlpRegisterParserAcceptableDataDirection(IPPROTO_TCP, ALPROTO_SMB, STREAM_TOSERVER);
    } else {
    ...
    if (AlpConfParserEnabled(proto_name)) {
        AlpRegisterParser(IPPROTO_TCP, ALPROTO_SMB, STREAM_TOSERVER, SMBParseRequest);
        AlpRegisterParser(IPPROTO_TCP, ALPROTO_SMB, STREAM_TOCLIENT, SMBParseResponse);
        AlpRegisterStateFuncs(IPPROTO_TCP, ALPROTO_SMB, SMBStateAlloc, SMBStateFree);
    } else {
...
int SMBParserTest01(void) {
    ...
    SCMutexLock(&f.m);
    int r = AlpParseL7Data(app_tctx, &f, ALPROTO_SMB, STREAM_TOSERVER|STREAM_EOF, smbbuf, smblen);
    if (r != 0) {
...
int SMBParserTest03(void) {
    ...
    SCMutexLock(&f.m);
    r = AlpParseL7Data(app_tctx, &f, ALPROTO_SMB, STREAM_TOSERVER|STREAM_START, smbbuf1, smblen1);
    if (r != 0) {
        printf("smb header check returned %" PRId32 ", expected 0: ", r);
        SCMutexUnlock(&f.m);
        goto end;
    ...
    SCMutexLock(&f.m);
    r = AlpParseL7Data(app_tctx, &f, ALPROTO_SMB, STREAM_TOSERVER, smbbuf2, smblen2);
    if (r != 0) {
        printf("smb header check returned %" PRId32 ", expected 0: ", r);
        SCMutexUnlock(&f.m);
        goto end;
    ...
    SCMutexUnlock(&f.m);
    SCMutexLock(&f.m);
    r = AlpParseL7Data(app_tctx, &f, ALPROTO_SMB, STREAM_TOSERVER, smbbuf3, smblen3);
    if (r != 0) {
        printf("smb header check returned %" PRId32 ", expected 0: ", r);
        SCMutexUnlock(&f.m);
        goto end;
...
int SMBParserTest04(void) {
    ...
    SCMutexUnlock(&f.m);
    SCMutexLock(&f.m);
    r = AlpParseL7Data(app_tctx, &f, ALPROTO_SMB, STREAM_TOSERVER, smbbuf4, smblen4);
    if (r != 0) {
        printf("smb header check returned %" PRId32 ", expected 0: ", r);
        SCMutexUnlock(&f.m);
        goto end;
...
int SMBParserTest07(void) {
    ...
    SCMutexLock(&f.m);
    r = AlpParseL7Data(app_tctx, &f, ALPROTO_SMB, STREAM_TOCLIENT | STREAM_START, smbbuf1, smblen1);
    if (r != 0) {
        printf("smb header check returned %" PRId32 ", expected 0: ", r);
        SCMutexUnlock(&f.m);
        goto end;
...
int SMBParserTest08(void) {
    ...
    SCMutexLock(&f.m);
    r = AlpParseL7Data(app_tctx, &f, ALPROTO_SMB, STREAM_TOCLIENT, smbbuf2, smblen2);
    if (r != 0) {
        printf("smb header check returned %" PRId32 ", expected 0: ", r);
        SCMutexUnlock(&f.m);
        goto end;
